...
byte getMaxDatesInFebruary(int year)
{
...
{
	byte mon = getTime().mon;
	int year = getTime().year;
	byte max_day_of_mon = 30;
	if (mon == 1 || mon == 3 || mon == 5 || mon == 7 || mon == 8 || mon == 10 || mon == 12)
		max_day_of_mon = 31;
	else if (mon == 2)
	{
		max_day_of_mon = getMaxDatesInFebruary(year);
	}
	if (date < 1)
		date = max_day_of_mon;
	else if (date > max_day_of_mon)
		date = 1;
	_writeRegister(REG_DATE, _encode(date));
}

void DS3231::validationDate(){
	byte mon = getTime().mon;
	int year = getTime().year;
	byte max_day_of_mon = 30;
	if (mon == 1 || mon == 3 || mon == 5 || mon == 7 || mon == 8 || mon == 10 || mon == 12)
	...
	if (getTime().date > max_day_of_mon)
	{
		_writeRegister(REG_DATE, _encode(max_day_of_mon));
	}
}

void DS3231::setMon(uint8_t mon)
{
	if (mon > 12)
		mon = 1;
	else if (mon < 1)
